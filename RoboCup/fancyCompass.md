	...
	int relativeSpeed = constrain(dist/20, 15, 60);
		...
		if(dist > 50){
			...
			SetLCD5Char(0, 0, angle, YELLOW, BLACK);
			    ...
			    if(vert == 0){
				if(hori == -1){
				    move(90, relativeSpeed);
				}else if(hori == 1){
				    move(270, relativeSpeed);
				}
			    }
			}else if(vert != 0){
			SetLCD5Char(0, 0, angle, YELLOW, BLACK);
			    ...
			    if(hori == 0){
				if(vert == -1){
				    move(180, relativeSpeed);
				}else{
				    move(0, relativeSpeed);
				}
			    }else if(hori == -1){
				if(vert == -1){
				    move(135, relativeSpeed);
				}else{
				    move(45, relativeSpeed);
				}
			    }else if(hori == 1){
				if(vert == -1){
				    move(225, relativeSpeed);
				}else{
				    move(315, relativeSpeed);
				}
			    }
			}
		}
...
void seeWhite(){	
	startTime=GetSysTimeMs();
		while(GetSysTimeMs()<(startTime+400)){
			compass = GetRemoIR(_FLAMEDETECT_compass_)/2800.0 * 360;
			goToPoint(0, 0);
		}
...
hori = 0;  //0 is not blind, -1 is left block, 1 is right block
	if(left-right < -750){
		hori = -1;
	}else if(left - right > 750){
		hori = 1;
	}

vert = 0; //0 is not blind, -1 is front block, 1 is back block
	if(front - back < -750){
		vert = -1; 
	}else if(front - back > 750){
		vert = 1;
	}
	
/*
	if((left -  right < 750 && left - right > 0)){
		isHoriBlind = 1;
	}else{
		isHoriBlind = 0;
	}
	if(abs(front - back) < 750){
		isVertiBlind = 1;
	}else{
		isVertiBlind = 0;
	}
...
/*
what i did: blind half works

to do: around ball is fucked???? when ball is behind the robot it just tweaks.
*/