...
int main(){
    ...
    vector<ll> originalQueries = queries;//store original order
    ...
    vector<ll> answers(Q);
    vector<pair<ll, int>> sorted(Q);
    for(int i = 0; i < Q; i++){
        sorted[i] = {originalQueries[i], i};
    }
    ...
    while(qIndex < Q){
        ...
        if(target <= cycleTime){
            ...
            answers[sorted[qIndex].second] = prefixWins[best];
            qIndex++;
        } 
        else {
            ll remainingTime = target - cycleTime;//number of full cycels
            ...
            answers[sorted[qIndex].second] = totalWins;
            qIndex++;
        }
    }
